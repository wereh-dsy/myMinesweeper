            ...
            for (int y = 0; y < cols_; y++)//遍历每个格子
            {
                ...
                if (!cell.isRevealed)//如果没有翻开过
                {
                    ...
                    if (cell.isFlagged)//如果插了旗子
                    {
                        ...
                        flag << QPoint(px, fr.top()) << QPoint(fr.right(), fr.top() + fr.height() / 4) << QPoint(px, fr.top() + fr.height() / 2);//加入三个点
...
void MainWindow::startSelection()
{
    ...
    {
    case 0: config.row = 6;  config.col = 6;  config.mines = 3;   break;
    case 1: config.row = 9;  config.col = 9;  config.mines = 10;  break;
    case 2: config.row = 16; config.col = 16; config.mines = 40;  break;
    case 3: config.row = 24; config.col = 24; config.mines = 100; break;
    case 4: config.row = 24; config.col = 30; config.mines = 180; break;
    default: config.row = 9; config.col = 9; config.mines = 10;   break;
    }//下拉框选项
